...
/*   Updated: 2024/03/29 03:05:55 by tiagoliv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execution(t_mini *mini, char **ev)
{
	...
	while (cmd)
	{
		...
		mini->if_cd = 0;
		set_execution(mini, cmd, ev, l_cmd && l_cmd->failed);
		l_cmd = cmd;
		cmd = cmd->next;
	}
...
void	pid_zero(t_command *cmd, char **ev, bool last_failed)
{
	(void)last_failed;
