...
bool	setup_redirections(t_command *cmd, bool isparent, bool dup)
{
	int		fd;
	t_redir	*redir;
	...
	while (redir != NULL)
	{
		...
		if (dup && (redir->type == RED_IN || redir->type == RED_AIN))
			dup2(fd, STDIN_FILENO);
		else if (dup && (redir->type == RED_OUT || redir->type == RED_AOUT))
